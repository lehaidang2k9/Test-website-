...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, sans-serif; }
        ...
        .del-btn { position: absolute; top: 1px; right: 1px; background: var(--danger); color: white; border: none; width: 18px; height: 18px; border-radius: 4px; z-index: 5; cursor: pointer; font-size: 12px; }
        #toast { position: fixed; top: 20px; right: -300px; background: var(--primary); color: white; padding: 10px 18px; border-radius: 10px 0 0 10px; transition: 0.4s; z-index: 10000; }
        #toast.show { right: 0; }
        .nav-bar { position: fixed; bottom: 0; left: 0; right: 0; background: rgba(17,17,24,0.8); backdrop-filter: blur(15px); border-top: 1px solid var(--border); display: flex; justify-content: space-around; padding: 12px 0; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-box"><small>Dung lượng</small><div class="stat-val">${(totalSizeBytes / 1024 / 1024).toFixed(1)}<small style="font-size:10px">MB</small></div></div>
        </div>
        <div class="upload-card">
            ...
            <div class="prog-wrap" id="pw">
                ...
                <div class="prog-text" id="pt" style="text-align:right; font-size:10px; margin-top:4px">0%</div>
            </div>
        </div>
        <div class="grid">
            ${files.map(f => `
                <div class="item-wrapper">
                    ...
                    <a href="${f.stored_name}" target="_blank" class="item">
                        ${f.mime_type.startsWith('image/') ? `<img src="${f.stored_name}" loading="lazy">` : `<div style="display:flex;align-items:center;justify-content:center;height:100%">🎬</div>`}
                    </a>
                </div>
            `).join('')}
        </div>
    ...
    <nav class="nav-bar">
        <a href="/" class="nav-item active"><span class="material-symbols-outlined">home</span><br><small>Home</small></a>
        <a href="#" class="nav-item"><span class="material-symbols-outlined">person</span><br><small>Profile</small></a>
    </nav>
